...
from app.config import (DEBUG, MAX_OVERFLOW, POOL_RECYCLE, POOL_SIZE,
                        POOL_TIMEOUT, SQLALCHEMY_DATABASE_URL, SSL_MODE)
...
class DatabaseHealthChecker:
    ...
    async def check_health(self, engine: AsyncEngine) -> bool:
        ...
        if current_time - self.last_check < required_interval:
            logger.debug(
                "Health check skipped (next check in {:.1f}s)",
                required_interval - (current_time - self.last_check),
            )
    ...
    def _reset_failure_tracking(self, current_time: float) -> None:
        if not self.is_healthy or self.consecutive_failures > 0:
            logger.info(
                "Database connection restored after {} failures",
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_check_in": max(
                0, self._calculate_backoff_interval() - (current_time - self.last_check)
            ),
        }
...
class DatabaseManager:
    ...
    async def _initialize(self, ssl_enabled: bool = True) -> None:
        async with self._init_lock:
            ...
            try:
                ...
                logger.success(
                    "Database initialized successfully (SSL={})", ssl_enabled
                )
            except Exception as e:
    ...
    async def verify_health(self) -> bool:
        return (
            await self._health_checker.check_health(self._engine)
            if self._engine
            else False
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                else DatabaseStatus.UNHEALTHY.value,
                "pool_size": POOL_SIZE,
                "max_overflow": MAX_OVERFLOW,
                "pool_timeout": POOL_TIMEOUT,
                "pool_recycle": POOL_RECYCLE,
